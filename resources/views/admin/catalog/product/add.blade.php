...
@section('content')
    <div class="admin container py-2">
        ...
        <div class="admin main-content p-4 table-responsive">
            <form class="mb-5" id="product-form" action="{{ route('admin.catalog.product.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                    ...
                    <div class="col-lg-6 mt-3">
                        <label for="brand" class="form-label align-self-end fw-bold">Category : <span
                                class="text-danger">*</span></label>
                                ...
                                @if(!$brand->children->isEmpty())
                                    <optgroup label="{{ $brand->title }}">
                                        @foreach ($brand->children as $child)
                                            <option value="{{ $child->id }}" @selected($child->id == old('brand_id'))>{{ $child->title }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                                @else
